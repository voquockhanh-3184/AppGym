...
const ExerciseDetailModal = ({ visible, onClose, exercise, onAddToSelection }: ExerciseDetailModalProps) => {
  ...
  const UniversalVideoPlayer = ({ uri, autoPlay = false, isLoop = false, showControls = true }: { uri: string, autoPlay?: boolean, isLoop?: boolean, showControls?: boolean }) => {
          ...
          return (
              <View style={styles.videoWrapper}>
                  <YoutubePlayer
                    height={width * 0.6} 
                    width={width}
                    // Nếu không hiện controls (Animation) thì luôn play, ngược lại theo state playing
                    play={!showControls ? true : playing} 
                    videoId={youtubeId}
                    ...
                    initialPlayerParams={{
                        controls: showControls, // Ẩn/Hiện controls YouTube
                        modestbranding: true,
                  ...
                  {/* Overlay điều khiển */}
                  {!showControls ? (
                      // Nếu ẩn controls -> Chặn tương tác để video chỉ hiển thị như ảnh động
                      <View style={styles.touchBlocker} />
                  ) : (
                      // Nếu hiện controls -> Hiện nút play overlay khi đang pause
      ...
      return (
        <>
            ...
            <Video
                ...
                muted={!showControls} // Mute nếu là animation loop
                repeat={isLoop}
                resizeMode="contain"
                paused={!showControls ? false : false} // Animation luôn chạy
                controls={showControls} // Ẩn/Hiện controls Native Video
  ...
  const renderMediaContent = () => {
    ...
    // Tab Hướng dẫn: Hiện Controls đầy đủ
    if (activeTab === 'instruction' && exercise.instruction_video) {
        return (
            <UniversalVideoPlayer 
                uri={exercise.instruction_video} 
                autoPlay={false}
                isLoop={false}
                showControls={true} // ✅ Hiện controls
            />
        );
    }

    // Tab Hoạt hình: Ẩn Controls, chạy loop
    if (exercise.video_path) {
        return (
             <UniversalVideoPlayer 
                uri={exercise.video_path} 
                autoPlay={true}
                isLoop={true}
